...
1. Loads study data from `../Data/study_scores.csv`
2. Fits a line `test_score = m * hours_studied + b` by minimizing mean squared error (MSE) via gradient descent
3. Trains for 1000 iterations, logging progress every 100 steps
4. Saves three plots to visualize the process and result
5. Uses the trained model to predict scores for a few example students
